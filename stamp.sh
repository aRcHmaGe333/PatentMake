...
if [ -z "$LICENSE_TYPE" ]; then
  ...
  echo "  apc    — canonical APC license"
  echo "  apc-vf — optional APC + ValueFlow profit sharing variant"
  read -rp "License type [apc/apc-vf]: " LICENSE_TYPE
fi

case "$LICENSE_TYPE" in
  apc)    LICENSE_SRC="$SCRIPT_DIR/LICENSE" ;;
  apc-vf) LICENSE_SRC="$SCRIPT_DIR/licenses/LICENSE-APC-VF.md" ;;
...
echo "Target:    $TARGET"
echo "Author:    $AUTHOR"
echo "License:   $LICENSE_TYPE -> $TARGET_LICENSE_FILE"
...
for CHECK_FILE in "$TARGET/$TARGET_LICENSE_FILE" "$TARGET/.github/workflows/timestamp.yml" "$TARGET/VERIFY.md"; do
...
cp "$LICENSE_SRC" "$TARGET/$TARGET_LICENSE_FILE"
...
sed -i.bak \
  ...
  "$TARGET/$TARGET_LICENSE_FILE"
rm -f "$TARGET/$TARGET_LICENSE_FILE.bak"

echo "Files copied and placeholders filled."

# --- Commit ---
git -C "$TARGET" add "$TARGET_LICENSE_FILE" VERIFY.md .github/workflows/timestamp.yml
git -C "$TARGET" commit -m "[APC] IPClaim stamp applied - $LICENSE_TYPE"
